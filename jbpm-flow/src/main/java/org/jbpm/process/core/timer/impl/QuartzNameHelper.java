...
class QuartzNameHelper {

    public static String getJobName(JobContext ctx) {
        String jobName;
        if (ctx instanceof ProcessJobContext) {
            ProcessJobContext processCtx = (ProcessJobContext) ctx;
            final String timerName = "-" + (processCtx.getTimer().getName() != null ? processCtx.getTimer().getName() + "-" : "") + processCtx.getTimer().getId();

            if (processCtx instanceof StartProcessJobContext) {
                jobName = "StartProcess-" + ((StartProcessJobContext) processCtx).getProcessId() + timerName;
            } else {
            ...
            jobName = "Timer-" + ctx.getClass().getSimpleName() + "-" + ctx.getJobHandle().getId();
        }
        return jobName;
    }
